...
public class Polynomial {
		...
		for (i = 0; i <= degree; i++) {
			...
			if (coeff[i] == 0) {
				...
				System.out.print(coeff[i] + "X"
						+ superscript(Integer.toString(expo[i])));
			} else if (expo[i] == 0) {
				System.out.print(sign + coeff[i]);
			}

			else {
				System.out.print(sign + coeff[i] + "X"
						+ superscript(Integer.toString(expo[i])));
			}
		}
		System.out.print("\n");
	...
	private int evalPoly() {
		...
		System.out.println("Enter value of 'x' to evaluate: ");
		valueToEvaluate = input.nextInt();
		// using horner method
		for (i = 1; i < n; i++)
			result = result * valueToEvaluate + coeff[i];
		return result;
	}
